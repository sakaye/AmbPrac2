...
if(isset($_POST['editSubbed'])){
	...
	$editUserObj->title_link = $_POST['eut_title'];
	$editUserObj->title = $_POST['newTitle'];
	$editUserObj->area_link = $_POST['eua_title'];
	$editUserObj->mob_link = $_POST['mobsList'];
	$editUserObj->mob = $_POST['otherMOB'];
	$editUserObj->departmentLink = $_POST['depts'];
	$editUserObj->dept = $_POST['otherDept'];
	$editUserObj->newDept = $_POST['newDeptTxt'];
	$editUserObj->newFacility = $_POST['newFacilityTxt'];
	editUserProfile($editUserObj);
...
function empStatusHandler(el){
	if(el.value == "true"){
		...
		document.getElementById("deptTxt").innerHTML='';
	} else {
...
<form action="user_dashboard_editProfile.php" method="post" name="edFrm" id="edFrm">
<!-- ============================== BASIC USER DATA ========================= -->
...
<div style="clear:both"></div>
      <div id="kpDept" class="npRow" style="border-top:solid 1px #999;">
        ...
        <p style="text-align:center"><span id="deptTxt"><?php echo $endUserObj->dept; ?></span> &nbsp;(<span style="cursor:pointer; text-decoration:underline; color:#039; font-size:12px" onclick="showDept()">change to...</span>)</p>
        <div style="clear:both"></div>
...
<div id="btnSub" style="float:right"><input name="Submit" type="submit" value="Submit" /></div>
<div style="clear:both" /></div>
