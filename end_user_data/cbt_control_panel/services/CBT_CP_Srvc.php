...
class CBT_CP_Srvc{
	...
	public function addQuestion($edID,$topicID,$question,$ansExp){
		...
		$safeQStr = str_replace('"',"'",$question);
		$safeQExpStr = str_replace('"',"'",$ansExp);
		
		$questionQ = 'INSERT INTO test_questions (tq_ed_link,tq_answer,tq_topic_area_link,tq_question,tq_pts_poss,tq_og_author)' .
				' VALUES ('. $edID . ',"' . $safeQExpStr . '",'. $topicID .',"'. $safeQStr .'",10,"'.$this->adminID.'")';
		$questionQResult = $db->query($questionQ);
	...
	public function addNewTestQuestion($ansObjs){
		...
		for($i=0;$i<count($ansObjs['ansArry']);$i++){
			$safeAStr = str_replace('"',"'",$ansObjs['ansArry'][$j]->answer);
			$insertAQ = 'INSERT INTO test_answers (ta_q_link,ta_answer,ta_correct) ' .
				'VALUES ('. $qID .',"'. $safeAStr .'","' . $ansObjs['ansArry'][$j]->correct . '")';
	...
	public function getAllTestQAndAs($edID){
		...
		$this->getAllAnswers();
		if($edID!=0){
			$quesQ = 'SELECT * FROM test_questions,qa_lookup WHERE qal_ed_link = ' . $edID . ' AND qal_q_link=test_questions.tq_id ORDER BY qal_rank, tq_id';
		} else {
	...
	public function editQAObj($qaObj,$task,$edID){
		...
		if($task=='edit'){
		$safeQStr = str_replace('"',"'",$qaObj->qData->question);
		$safeQExpStr = str_replace('"',"'",$qaObj->qData->ansExp);
		
		$qEditQ = 'UPDATE test_questions SET tq_question="'. $safeQStr . '", tq_answer="'. $safeQExpStr .'" ' .
				'WHERE tq_id=' . $qaObj->qData->qID . ' LIMIT 1';
		$qEditQResult = $db->query($qEditQ);
		
		for($i=0;$i<count($ansObjArry);$i++){
			$safeAStr = str_replace('"',"'",$ansObjArry[$i]->answer);
			$ansUpdateQ = 'UPDATE test_answers SET ta_answer="' . $safeAStr . '", ta_correct="'. $ansObjArry[$i]->correct .'" ' .
